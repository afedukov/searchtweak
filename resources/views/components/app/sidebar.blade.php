					...
					<li class="px-3 py-2 rounded-sm mb-0.5 last:mb-0 @if(in_array(Request::segment(1), ['admin'])){{ 'bg-slate-900' }}@endif" x-data="{ open: {{ in_array(Request::segment(1), ['admin']) ? 1 : 0 }} }">
						...
						<div class="lg:hidden lg:sidebar-expanded:block 2xl:block">
							<ul class="pl-9 mt-1 @if(!in_array(Request::segment(1), ['admin'])){{ 'hidden' }}@endif" :class="open ? '!block' : 'hidden'">
								<li class="mb-1 last:mb-0">
									<a class="block text-slate-400 hover:text-slate-200 transition duration-150 truncate @if(Route::is('superuser.dashboard')){{ '!text-indigo-500' }}@endif" href="{{ route('superuser.dashboard') }}">
								...
								<li class="mb-1 last:mb-0">
									<a class="block text-slate-400 hover:text-slate-200 transition duration-150 truncate @if(Route::is('superuser.settings')){{ '!text-indigo-500' }}@endif" href="{{ route('superuser.settings') }}">
										<span class="text-sm font-medium lg:opacity-0 lg:sidebar-expanded:opacity-100 2xl:opacity-100 duration-200">Settings</span>
									</a>
								</li>
								<li class="mb-1 last:mb-0">
